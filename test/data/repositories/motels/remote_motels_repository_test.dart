...
void main() {
  ...
  group('RemoteMotelsRepository - getGoNowList', () {
    test('should return a list of motels when the request is successful',
      ...
      expect(motels.first.name, 'Motel Le Nid'); // Ensure correct parsing
    });

    test(
        'should return a FormatedException when the response format is incorrect',
        () async {
      // Arrange: Mock invalid data format
    ...
    test(
        ...
        () async {
      // Arrange: Simulate client throwing an exception
    ...
    test('should handle unexpected errors gracefully', () async {
      ...
      when(() => mockClientHttp.getGoNowList()).thenAnswer(
          (_) async => asyncOutputFailure(exception)); // Return a failed future
